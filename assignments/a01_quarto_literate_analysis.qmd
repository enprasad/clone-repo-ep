...
# Why we Care About the Redfield Ratio

The Redfield Ratio is a ratio that measures the concentration of nitrogen and phosphorus against each other, and the typical ratio of N/P is around 16:1 for healthy marine environments. In the paper, it said that this ratio is a result of plankton on the surface of the ocean interacting with the nutrients that come from the deep ocean. When this ratio is thrown off, it can be an indication that something is wrong with the ocean environment the sample came from. It can be an indication of global warming or an imbalance within the ocean.
...
```{r}
bg_chem_dat <- read.csv(file = "C:/Users/epras/OneDrive/Desktop/BIO806/import-template-enprasad_2/data/raw/BGchem2008data.csv")
```
...
```{r}
nitrate <- mean(bg_chem_dat$NO3, na.rm = TRUE)
nitrite <- mean(bg_chem_dat$NO2, na.rm = TRUE)
amm     <- mean(bg_chem_dat$NH4, na.rm = TRUE)
phos    <- mean(bg_chem_dat$P,   na.rm = TRUE)
```
...
plot(
  bg_chem_dat$P,
  bg_chem_dat$NO2 + bg_chem_dat$NO3 + bg_chem_dat$NH4,
  xlab = "Phosphate (P)",
...
This plot shows the ratio of nitrogen to phosphorous in this dataset at different locations. There are some outliers that have a high nitrogen ratio compared to a lower amount of phosphorous. These points of data could come from deep ocean analysis, where nitrogen concentration is much greater than when sampling is done closer to the surface.
